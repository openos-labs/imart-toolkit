...
import { personFallEvent, initPositionEvent } from '../../utils/bvhEvent';
...
createEffect(
	function (this: PhysicsObjectManager) {
		...
		const handle = onBeforeRender(() => {
			...
			for (const characterManager of bvhCharacterSet) {
				...
				if (centripetal) {
					playerVelocity.add(
						characterManager.bvhOnGround || characterManager._gravity === false
							? vector3_0
					...
					box3.setFromCenterAndSize(
						player.position,
						vector3__.set(capsuleRadius * 2, capsuleHalfHeight * 2, capsuleRadius * 2)
					);
						...
						boundsTree.shapecast({
							...
							intersectsTriangle: tri => {
								...
								if (distance < capsuleRadius) {
									...
									if (
										Math.abs(triPoint.y).toFixed(3) === Math.abs(tri.a.y).toFixed(3) ||
										Math.abs(triPoint.y).toFixed(3) === Math.abs(tri.b.y).toFixed(3) ||
										Math.abs(triPoint.y).toFixed(3) === Math.abs(tri.c.y).toFixed(3)
									) {
										contact = true;
									} else {
										contact = false;
					...
					if (contact && mapManager) forceGet(bvhContactMap, characterManager, makeWeakSet).add(mapManager);

					const deltaVector = start.sub(startOld);
					if (centripetal) characterManager.bvhOnGround = contact;
					else {
						characterManager.bvhOnGround = contact || deltaVector.y > Math.abs(delta * playerVelocity.y * 0.25);

						if (
							repulsion &&
							characterManager.bvhOnGround &&
							Math.abs(deltaVector.y / (deltaVector.x + deltaVector.z + Number.EPSILON)) < repulsion
						)
					...
					if (characterManager.bvhOnGround && global.isFirstLoad) {
						...
						const { _firstInnerY, _firstInnerX, _firstInnerZ } = characterManager as any;
						...
						//emit value
						personFallEvent.emit(player.position);
						initPositionEvent.listener(() => {
							const { _firstInnerY, _firstInnerX, _firstInnerZ } = characterManager as any;
	...
	[getBVHMap, getGravity, getRepulsion, getCentripetal, getEditorPlay, getFirstLoad, getBVHComputing]
);
