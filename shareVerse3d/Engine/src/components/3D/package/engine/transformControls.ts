...
createEffect(() => {
	...
	const target =
		getSelectionNativeTarget() ?? (isPositionedItem(selectionTarget) ? selectionTarget.outerObject3d : undefined);
	...
	lazyTransformControls().then(transformControls => {
		if (handle.done || !target.parent || (mode !== 'translate' && mode !== 'rotate' && mode !== 'scale')) return;
