...
import {NftLottery as NFTLOTTERY,NftLottery__factory,NftToken__factory,NftToken} from "../typechain";
import {
    Address,
    Config,
    Tx,
...
export class NftLottery implements NftLotteryInterface{
    ...
    async setApprovalForAll(_nftContractAddress:string,_operator: string, _approved: boolean, signer?: any): Promise<Tx> {
        return this.erc721(_nftContractAddress).connect(signer??this.signer).setApprovalForAll(_operator, _approved);
    }

    async createActivity(_nftContractAddress: string, _endBlockNumber: number, _activityId: number, _tokenIds: number[], signer?: any): Promise<any> {
        return this.nftLottery()
            .connect(signer??this.signer)
            .createActivity(_nftContractAddress, _endBlockNumber, _activityId, _tokenIds);
    }
    async setMerkleRoot(_activityId: number, _merkleRoot: string, signer?: any): Promise<any> {
        return this.nftLottery()
            .connect(signer??this.signer)
            .setMerkleRoot(_activityId, _merkleRoot);
    }
    async claim(_organizer: string, _activityId: number, _nftContract: string, _tokenId: number, merkleProof: string[], signer?: any): Promise<any> {
        return this.nftLottery()
            .connect(signer??this.signer)
    ...
    async getRemainingTokenIds(_organizer: string, _activityId: number): Promise<any> {
